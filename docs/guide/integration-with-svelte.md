...
HyperFormula has no browser-only API dependencies, but constructing the engine inside SvelteKit's server-rendered output is wasted work. In SvelteKit, initialize the engine inside `onMount` so it only runs in the browser:
